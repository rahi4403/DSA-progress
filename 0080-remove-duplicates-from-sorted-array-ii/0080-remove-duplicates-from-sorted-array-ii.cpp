...
public:
    int removeDuplicates(vector<int>& nums) {
        ...
        if(nums.size()<=2){ //if the array has only 2 or less than that element then we return the size of the array itself,it has to be checked before the loop since otherwise it directly returns 2 from the for loop
                return nums.size();
            }
